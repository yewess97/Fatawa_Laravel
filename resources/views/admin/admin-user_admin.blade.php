...
@section('content')
    <div class="super_container">
        ...
        <!--Add sheikh modal-->
        <div class="modal fade" id="addSheikh" tabindex="-1" role="dialog" aria-hidden="true">
            <div class="modal-dialog modal-lg" role="document">
                <div class="modal-content border-0 p-4">
                    <div class="modal-header border-0">
                        <h3 style="font-family: Cairo,'El Messiri';">{{__('all.register_sheikh')}}</h3>
                    ...
                    <div class="modal-body">
                        <div class="login">
                            <form action="{{route('add_sheikh')}}" class="row form-register" method="post"
                                ...
                                <div class="col-12">
                                    <label class="col-12 required">{{__('all.specialize')}} &nbsp;</label>
                                    <select class="input form-control mb-3" aria-required="true"
                                            style="cursor: pointer" name="sheikh_specialize" required>
        ...
        @foreach($sheikhs as $sheikh)
            <div class="modal fade" id="editSheikh{{$sheikh->id}}" tabindex="-1" role="dialog" aria-hidden="true">
                <div class="modal-dialog modal-lg" role="document">
                    <div class="modal-content border-0 p-4">
                        <div class="modal-header border-0">
                            <h3 style="font-family: Cairo, 'El Messiri';">{{__('all.edit_sheikh')}}</h3>
                            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                <span aria-hidden="true">&times;</span>
                            </button>
                        </div>
                        ...
                        <div class="modal-body">
                            <div class="login">
                                <form action="{{route('update_sheikh', $sheikh->id)}}" class="row form-register"
                                    ...
                                    <div class="col-6 mb-4">
                                        ...
                                        <select class="input form-control"
                                                ...
                                                name="gender">
                                            <option value="male"
                                                    @if($sheikh->gender == 'male') selected @endif>{{__('all.male')}}</option>
                                            <option value="female"
                                                    @if($sheikh->gender == 'female') selected @endif>{{__('all.female')}}</option>
                                        </select>
                                    </div>
                                    <div class="col-6 mb-4">
                                        <label
                                    ...
                                    <div class="col-6 mb-4">
                                        ...
                                        <select class="input form-control"
                                                ...
                                                style="cursor: pointer"
                                                name="social_status">
                                            @if($sheikh->gender == 'male')
                                                <option value="single"
                                                        @if($sheikh->social_status == 'single') selected @endif>{{__('all.single_male')}}</option>
                                                <option value="engaged"
                                                        @if($sheikh->social_status == 'engaged') selected @endif>{{__('all.engaged_male')}}</option>
                                                <option value="married"
                                                        @if($sheikh->social_status == 'married') selected @endif>{{__('all.married_male')}}</option>
                                                <option value="widower"
                                                        @if($sheikh->social_status == 'widower') selected @endif>{{__('all.widower_male')}}</option>
                                                <option value="divorced"
                                                        @if($sheikh->social_status == 'divorced') selected @endif>{{__('all.divorced_male')}}</option>
                                            @else
                                                <option value="single"
                                                        @if($sheikh->social_status == 'single') selected @endif>{{__('all.single_female')}}</option>
                                                <option value="engaged"
                                                        @if($sheikh->social_status == 'engaged') selected @endif>{{__('all.engaged_female')}}</option>
                                                <option value="married"
                                                        @if($sheikh->social_status == 'married') selected @endif>{{__('all.married_female')}}</option>
                                                <option value="widower"
                                                        @if($sheikh->social_status == 'widower') selected @endif>{{__('all.widower_female')}}</option>
                                                <option value="divorced"
                                                        @if($sheikh->social_status == 'divorced') selected @endif>{{__('all.divorced_female')}}</option>
                                            @endif
                                        </select>
                                    </div>
                                    <div class="col-6">
                                        ...
                                        <div class="text-center mt-2">
                                            <img src="{{asset($sheikh->users_image)}}"
                                                 class="rounded-circle img-fluid img-users">
                                        </div>
                                    </div>
                                    <div class="col-12">
                                        <label class="col-12">{{__('all.specialize')}}</label>
                                        <select class="input form-control mb-3" aria-required="true"
                                                style="cursor: pointer"
                                            ...
                                            @foreach($specializes as $specialize)
                                                @if(app()->getlocale() == 'ar')
                                                    <option
                                                        value="{{$specialize->id}}"
                                                        @if($specialize->id == $sheikh->specialize_id) selected @endif>{{$specialize->name_ar}}</option>
                                                @else
                                                    <option
                                                        value="{{$specialize->id}}"
                                                        @if($specialize->id == $sheikh->specialize_id) selected @endif>{{$specialize->name_en}}</option>
                                                @endif
                                            @endforeach
                                        </select>
                                    </div>
        ...
        <section id="admins" class="admin-section">
            <div class="container">
                        ...
                        <table class="table table-bordered table-striped w-100">
                            ...
                            @foreach($sheikhs as $sheikh)
                                <tr style="font-family: DroidArabicNaskh; line-height: 1.8rem">
                                    ...
                                    <td>
                                        @if($sheikh->gender == 'male')
                                            @if($sheikh->social_status == 'single')
                                                {{__('all.single_male')}}
                                            @elseif($sheikh->social_status == 'engaged')
                                                {{__('all.engaged_male')}}
                                            @elseif($sheikh->social_status == 'married')
                                                {{__('all.married_male')}}
                                            @elseif($sheikh->social_status == 'widower')
                                                {{__('all.widower_male')}}
                                            @elseif($sheikh->social_status == 'divorced')
                                    ...
                                    @if($sheikh->specialize != null)
                                        <td>
                                            @if(app()->getlocale() == 'ar')
                                                {{$sheikh->specialize->name_ar}}
                                            @else
                                                {{$sheikh->specialize->name_en}}
                                            @endif
                                        </td>
                                    @else
                                        <td>{{__('all.deleted_specialize')}}</td>
                                    @endif
                                    <td dir="ltr">{{$sheikh->email}}</td>
                                    <td dir="ltr">{{$sheikh->phone}}</td>
                                        ...
                                        <div class="col-12 d-flex text-center border-bottom-0"
                                            ...
                                            <div class="col-6">
                                                <button type="button" class="button btn btn-info"
                                                        data-toggle="modal"
                                                        data-target="#editSheikh{{$sheikh->id}}">{{__('all.edit')}}
                                                </button>
                                            </div>
                                            <div class="col-6">
                                                <a href="{{route('delete_sheikh', $sheikh->id)}}" type="button"
                ...
                <button type="button" class="btn btn-primary float-left pl-5 pr-5 mt-3 ml-4" data-toggle="modal"
                        data-target="#addSheikh">{{__('all.add_sheikh')}}
                </button>
            </div>
        </section>
    </div>
